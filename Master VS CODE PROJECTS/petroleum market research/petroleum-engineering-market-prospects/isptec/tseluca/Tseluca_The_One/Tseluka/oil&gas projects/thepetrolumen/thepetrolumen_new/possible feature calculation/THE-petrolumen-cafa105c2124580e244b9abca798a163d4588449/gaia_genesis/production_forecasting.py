...
class ProductionForecasting:
    ...
    def fit_decline_curves(self,
        ...
        if len(well_data) < min_data_points:
            raise ValueError(f"Dados insuficientes para ajuste (mínimo: {min_data_points})")
    ...
    def forecast_production(self,
                          well_name: str,
                          forecast_period: int,
                          decline_type: Optional[str] = None) -> pd.DataFrame:
        """
        ...
        t_last = self.historical_data[self.historical_data['well_name'] == well_name]['time'].max()
    ...
    def generate_development_scenarios(self,
                                    reservoir_name: str,
                                    num_scenarios: int,
    ...
    def plot_decline_curve(self, well_name: str):
        ...
        if params['type'] == 'exponential':
            ...
            q_fit = params['qi'] * (1 + params['b'] * params['Di'] * t_fit)**(-1/params['b'])
        else:  # harmonic
    ...
    def plot_material_balance(self, reservoir_name: str):
        ...
        if reservoir_name not in self.material_balance:
            raise ValueError(f"Balanço de materiais não encontrado para {reservoir_name}")
    ...
    def plot_scenarios(self, reservoir_name: str):
        ...
        for i, scenario in enumerate(self.scenarios[reservoir_name]):
            plt.subplot(2, 2, i+1)
            plt.scatter(scenario['well_positions'][:,0],
    ...
    def _calculate_fetkovich_aquifer(self,
                                   p: np.ndarray,
                                   pvt_data: Dict) -> np.ndarray:
    ...
    def _calculate_carter_tracy_aquifer(self,
                                      p: np.ndarray,
                                      pvt_data: Dict) -> np.ndarray:
    ...
    def _calculate_scenario_production(self,
                                    num_wells: int,
    ...
    def _calculate_polymer_recovery(self,
                                  concentration: float,
    ...
    def _calculate_surfactant_recovery(self,
                                     concentration: float,
                                     salinity: float,
                                     constraints: Dict) -> float:
    ...
    def _calculate_thermal_recovery(self,
                                  temperature: float,
                                  injection_rate: float,
                                  constraints: Dict) -> float:
        """Calcula recuperação para recuperação térmica."""
        # Parâmetros
        mu_oil = constraints['oil_viscosity']
        mu_steam = constraints['steam_viscosity']
