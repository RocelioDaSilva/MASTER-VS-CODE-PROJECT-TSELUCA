...
export default function App() {
  ...
  const [compareResults, setCompareResults] = useState<SuitabilityResult[]>([]);
  ...
  // Fetch results for the comparison scenario when selected
  useEffect(() => {
    if (!compareScenario) { setCompareResults([]); return; }
    const fetchCompare = async () => {
      try {
        const response = await fetch('/api/analyze', {
          ...
          body: JSON.stringify({
            ...
            weights: compareScenario.weights,
            params: compareScenario.params,
          }),
        });
        if (response.ok) {
          const data = await response.json();
      ...
      } catch { /* comparison is non-critical */ }
    };
    fetchCompare();
  }, [compareScenario]);
                ...
                <div className="space-y-8">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                    ...
                    <StatCard 
                      ...
                      value={results.length > 0 ? (results.reduce((acc, r) => {
                        const c = ANGOLA_COMMUNITIES.find(com => com.id === r.communityId);
                        return acc + (c ? c.ghi * c.population * 0.001 : 0);
                      }, 0) / 1000).toFixed(1) : '...'}
                      unit="GW"
                    />
                  </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[600px]">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="opacity-80 grayscale-[0.2]">
                          <Charts results={compareResults.length > 0 ? compareResults : results} />
                        </div>
                      </div>
                    </div>
                  ) : (
