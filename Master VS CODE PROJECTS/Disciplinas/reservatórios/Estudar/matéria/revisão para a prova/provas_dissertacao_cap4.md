...
1) Discuta as principais fontes de incerteza em uma estimativa volumétrica de OOIP (A, h_net, φ, S_wi, B_o). Proponha estratégias práticas para reduzir cada fonte de incerteza.
...
4) Explique como construir mapas de Net‑to‑Gross e A_eff; discuta como escolhas de cut‑off de porosidade/permeabilidade afetam a estimativa de reservas.

5) Discuta a sensibilidade do OOIP a variações em φ versus variações em B_o. Use notação matemática/um exemplo numérico curto para ilustrar.
...
- densidade do óleo: ρ_o = 0.84 g/cm³
- volume total da amostra: V_tot = 180 cm³
...
Arredondando para 3 algarismos significativos: V_f ≈ 29.8 cm³.
...
- Volume de fluido nos poros: V_f ≈ 29.8 cm³
- Porosidade: φ ≈ 16.5 %

**Observações**
- Unidades usadas: g, cm³, g/cm³. Para SI, converta massa (kg) e volume (m³) consistentemente.  
- Método alternativo (método de Arquímedes) e outros exemplos estão disponíveis em `exercicios_resolvidos.md` e no material do Cap.3.
...
(usar conversões quando q em STB/d, Âµ em cP, k em mD — ver fórmulas de campo no anexo)
...
4.1. Fórmula fundamental (reservoir → superfíce)
...
- Construir tabela com p, N_p, G_p, B_o(p),B_g(p),R_s(p), calcular $E$'s e ajustar. (ver tarefas do capitulo 5 para exemplo completo do curso).
...
- Cap.1: listar elementos do sistema petrolífero; identificar tipo de armadilha e presença de gas‑cap/ aquífero.
- Cap.2: ter curvas B_o(p), R_s(p), Âµ_o(p) e tabela PVT; conhecer EoS e quando aplicá‑las.
...
Se desejar, posso expandir esta seção com soluções de todos os exercícios presentes em `exercicios_transcription.md` (isto exigirá mais tempo e poderei criar um ficheiro separado `exercicios_resolvidos.md`).
