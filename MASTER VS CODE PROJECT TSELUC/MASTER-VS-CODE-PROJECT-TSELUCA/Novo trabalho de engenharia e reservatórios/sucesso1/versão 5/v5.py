...
"""
PetroChamp - Enhanced Oil Recovery (EOR) Screening Platform
Version: 5.0 - Refactored with improved structure and documentation
"""
...
class SuitabilityLevel(Enum):
    """Enum for suitability classification"""
    ALTA = (80, 100, '#27ae60', '🟢')      # Green
    MEDIA = (60, 79, '#f39c12', '🟡')       # Orange
    BAIXA = (0, 59, '#e74c3c', '🔴')        # Red


@dataclass
class ColorScheme:
    """Centralized color configuration"""
    alta: str = '#27ae60'      # Green
    media: str = '#f39c12'     # Orange
    baixa: str = '#e74c3c'     # Red
    neutro: str = '#bdc3c7'    # Gray
...
class ScreeningConfig:
    """Screening thresholds and weights"""
    THRESHOLD_ALTA = 80
    THRESHOLD_MEDIA = 60
    DECIMAL_PLACES = 1
    DEFAULT_WEIGHT = 0.1
...
class SuitabilityVisualizer:
    """Handles all visualization of suitability scores and comparisons"""
    
    def __init__(self, config: Optional[ColorScheme] = None):
        """Initialize visualizer with optional custom color scheme"""
        self.colors = config or ColorScheme()
        self.viz_config = VisualizationConfig()
        self._setup_matplotlib()
    
    def _setup_matplotlib(self) -> None:
        """Configure matplotlib style and appearance"""
        try:
            plt.style.use(self.viz_config.DEFAULT_STYLE)
            sns.set_palette(self.viz_config.DEFAULT_PALETTE)
    ...
    def _get_bar_colors(self, scores: List[float]) -> List[str]:
        """Get colors for bars based on scores"""
        return [self._get_score_color(score) for score in scores]
    ...
    def create_spider_chart(self, method_scores: Dict[str, Dict], 
        ...
        color_map = {'green': self.colors.alta, 'orange': self.colors.media, 
        ...
        ax1.axvline(x=ScreeningConfig.THRESHOLD_MEDIA, color='orange', linestyle='--', 
                   alpha=0.5, label='Média (60-80%)')
        ax1.axvline(x=0, color='red', linestyle='--', alpha=0.5, label='Baixa (<60%)')
        ax1.legend()
        
        # Gráfico 2: Radar para top 3 métodos
        self._plot_top_methods_radar(ax2, method_scores)
    ...
    def _plot_top_methods_radar(self, ax: Any, method_scores: Dict[str, Dict]) -> None:
        """Plot radar chart for top 3 methods"""
        ...
        for i, (method, data) in enumerate(top_methods):
            ...
            ax.plot(angles, val, linewidth=self.viz_config.LINEWIDTH_DEFAULT, 
                   linestyle='solid', label=method[:15])
            ax.fill(angles, val, alpha=self.viz_config.ALPHA_DEFAULT)
        
        ax.set_theta_offset(np.pi / 2)
        ax.set_theta_direction(-1)
        ax.set_thetagrids(np.degrees(angles[:-1]), categories)
        ax.set_ylim(0, 100)
        ax.set_title('Análise Multidimensional - Top 3 Métodos')
...
class ReservoirDataValidator:
    """Validates and cleans reservoir data"""
    
    REQUIRED_FIELDS = {'Óleo API', 'Viscosidade', 'Profundidade'}
    OPTIONAL_FIELDS = {
        'Espessura', 'Saturação de Óleo', 'Permeabilidade', 'Pressão',
        'Temperatura', 'Salinidade', 'pH', 'TAN', 'Saturação de Água', 'Dip'
    }
    ...
    def validate_data(cls, data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for field, value in data.items():
            ...
            min_val, max_val = cls.VALID_RANGES.get(field, (0, 1000000))
    ...
    def sanitize_data(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        """Remove invalid fields and convert to proper types"""
        sanitized = {}
        for field, value in data.items():
            if field not in cls.VALID_FIELDS:
            ...
            try:
                ...
                logger.error(f"Could not convert {field}={value} to float")
                continue
...
class FileHandler:
    ...
    @staticmethod
    def save_file(data: Any, filepath: str, file_format: str = 'xlsx') -> bool:
...
# ============================================================================
# MÓDULO DE TRIAGEM EOR AVANÇADA COM JUSTIFICAÇÕES COMPLETAS - CORRIGIDO
# ============================================================================
        ...
        return {
            ...
            "Injeção de Surfactantes": {
                "alta": (
                    ...
                    "temperatura adequada (<80°C) para estabilidade química, e saturação residual de óleo (>25%) que justifica o cost dos surfactantes. "
            ...
            "Injeção Microbiana": {
                "alta": (
                    ...
                    "e pH dentro dos limites (5-9) para crescimento microbiano. Microrganismos produzem surfactantes, polímeros, gases e ácidos in situ. "
...
# ============================================================================
# MÓDULO DE ANÁLISE ECONÔMICA COMPLETO
# ============================================================================
...
# ============================================================================
# PLATAFORMA PRINCIPAL - PETROCHAMP PLATFORM COMPLETA
# ============================================================================
class PetroChampPlatform:
    ...
    def _create_dashboard_tab(self):
        ...
        ttk.Button(actions_frame, text="Gerar Dashboard Suitability", 
                  command=self.show_suitability_dashboard, width=25).pack(side=tk.LEFT, padx=5)
    ...
    # ============================================================================
    # MÉTODOS DE SUITABILITY - COMPLETOS
    # ============================================================================
    ...
    # ============================================================================
    # MÉTODOS DE INTERFACE - COMPLETOS
    # ============================================================================
