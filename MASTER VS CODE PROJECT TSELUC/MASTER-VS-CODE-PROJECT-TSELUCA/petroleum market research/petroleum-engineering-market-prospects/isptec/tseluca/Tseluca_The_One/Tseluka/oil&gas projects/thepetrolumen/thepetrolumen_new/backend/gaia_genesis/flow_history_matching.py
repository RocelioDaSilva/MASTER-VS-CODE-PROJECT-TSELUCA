...
import numpy as np
from typing import Dict  # Removed Optional
...
class FlowHistoryMatching(HistoryMatching):
    ...
    def update_simulation_parameters(self):
        ...
        if "permeability_multiplier" in self.parameters:
            self.simulation.permeability *= self.parameters["permeability_multiplier"][
                "value"
            ]
        ...
        if "sor" in self.parameters:
            self.simulation.saturation["oil"] = np.maximum(
                self.simulation.saturation["oil"], self.parameters["sor"]["value"]
            )

        if "swc" in self.parameters:
            self.simulation.saturation["water"] = np.maximum(
                self.simulation.saturation["water"], self.parameters["swc"]["value"]
            )

        if "sgc" in self.parameters:
            self.simulation.saturation["gas"] = np.maximum(
                self.simulation.saturation["gas"], self.parameters["sgc"]["value"]
            )

        # Atualizar parâmetros de permeabilidade relativa
        if "n_o" in self.parameters:
            self.simulation.relative_permeability["n_o"] = self.parameters["n_o"][
                "value"
            ]

        if "n_w" in self.parameters:
            self.simulation.relative_permeability["n_w"] = self.parameters["n_w"][
                "value"
            ]

        if "n_g" in self.parameters:
            self.simulation.relative_permeability["n_g"] = self.parameters["n_g"][
                "value"
            ]

        # Atualizar parâmetros de pressão capilar
        if "lambda_bc" in self.parameters:
            self.simulation.capillary_pressure["lambda_bc"] = self.parameters[
                "lambda_bc"
            ]["value"]
        ...
        if "aquifer_volume_multiplier" in self.parameters:
            self.simulation.aquifer_properties["aquifer_volume"] *= self.parameters[
                "aquifer_volume_multiplier"
            ]["value"]

        if "aquifer_compressibility_multiplier" in self.parameters:
            self.simulation.aquifer_properties[
        ...
        if "gas_cap_volume_multiplier" in self.parameters:
            self.simulation.gas_cap_properties["gas_cap_volume"] *= self.parameters[
                "gas_cap_volume_multiplier"
            ]["value"]

        if "gas_oil_contact_shift" in self.parameters:
            self.simulation.gas_cap_properties["gas_oil_contact"] += self.parameters[
                "gas_oil_contact_shift"
            ]["value"]

        # Atualizar parâmetros de gás em solução
        if "initial_gas_oil_ratio_multiplier" in self.parameters:
            self.simulation.solution_gas_properties[
                "initial_gas_oil_ratio"
