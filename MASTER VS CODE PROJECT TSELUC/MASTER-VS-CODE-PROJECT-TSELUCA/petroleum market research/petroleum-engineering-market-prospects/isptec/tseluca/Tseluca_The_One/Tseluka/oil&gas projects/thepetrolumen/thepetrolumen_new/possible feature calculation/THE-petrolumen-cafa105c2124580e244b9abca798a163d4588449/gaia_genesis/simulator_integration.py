...
class SimulatorIntegration:
    ...
    def _initialize_integration(self):
        """Initialize integration based on simulator type."""
        self.logger.info(f"Initializing integration for {self.simulator.simulator_type} simulator")
        ...
        if hasattr(self.simulator, 'simulation_type'):
            from .commercial_simulator import SimulationType
            
            if self.simulator.simulation_type == SimulationType.COUPLED_GEOMECHANICS:
                self._initialize_geomechanics(grid_dims)
            
            if self.simulator.simulation_type == SimulationType.THERMAL:
                self._initialize_thermal(grid_dims)
            
            if self.simulator.simulation_type == SimulationType.COMPOSITIONAL:
                self._initialize_compositional()
            
            if self.simulator.simulation_type in [SimulationType.DUAL_POROSITY, SimulationType.DUAL_PERMEABILITY]:
    ...
    def _initialize_geomechanics(self, grid_dims: Tuple[int, int, int]):
        ...
        # Default properties
        properties = {
            'youngs_modulus': np.ones(grid_dims[0] * grid_dims[1] * grid_dims[2]) * 1e6,  # psi
            'poissons_ratio': np.ones(grid_dims[0] * grid_dims[1] * grid_dims[2]) * 0.25,
            'biot_coefficient': 1.0,
            'cohesion': np.ones(grid_dims[0] * grid_dims[1] * grid_dims[2]) * 1000.0,  # psi
            'friction_angle': np.ones(grid_dims[0] * grid_dims[1] * grid_dims[2]) * 30.0  # degrees
        }
    ...
    def _initialize_thermal(self, grid_dims: Tuple[int, int, int]):
        ...
        properties = {
            ...
            'rock_heat_capacity': np.ones(grid_dims[0] * grid_dims[1] * grid_dims[2]) * 0.2,  # Btu/lb-°F
            'rock_thermal_conductivity': np.ones(grid_dims[0] * grid_dims[1] * grid_dims[2]) * 1.0,  # Btu/ft-hr-°F
            'rock_density': np.ones(grid_dims[0] * grid_dims[1] * grid_dims[2]) * 165.0  # lb/ft³
        }
        ...
        if hasattr(self.simulator, 'thermal_data') and 'steam_injection' in self.simulator.thermal_data:
    ...
    def _initialize_compositional(self):
        ...
        if hasattr(self.simulator, 'compositional_data') and 'components' in self.simulator.compositional_data:
        ...
        eos_type = self.simulator.compositional_data.get('eos', 'PR') if hasattr(self.simulator, 'compositional_data') else 'PR'
    ...
    def _initialize_dual_continuum(self):
        ...
        if self.simulator.grid is not None and 'dual_properties' in self.simulator.grid:
            matrix_props = {
                'porosity': self.simulator.grid['dual_properties'].get('matrix_porosity', 0.1),
                'permeability': self.simulator.grid['dual_properties'].get('matrix_permeability', 0.1),
                'shape_factor': self.simulator.grid['dual_properties'].get('sigma', 1e-7)
            }
            
            fracture_props = {
                'porosity': self.simulator.grid['dual_properties'].get('fracture_porosity', 0.01),
                'permeability': self.simulator.grid['dual_properties'].get('fracture_permeability', 1000.0),
    ...
    def run_integrated_simulation(self, timesteps: int, dt: float) -> Dict:
        ...
        for step in range(timesteps):
            self.logger.info(f"Timestep {step+1}/{timesteps}, simulation time: {current_time:.2f} days")
            
            # Run basic flow simulation
            if hasattr(self.simulator, 'run_simulation'):
                self.simulator.run_simulation(1, dt)
            
            # Run geomechanical simulation if enabled
            if self.geomechanics and hasattr(self.simulator, 'grid'):
                pressure = self.simulator.simulation_results.get(f'timestep_{step}', {}).get('pressure', None)
                if pressure is not None:
                    ...
                    if 'properties' in self.simulator.grid:
                        initial_porosity = self.simulator.grid['properties'].get('porosity', None)
                        initial_permeability = self.simulator.grid['properties'].get('permeability_x', None)
                        
                        if initial_porosity is not None and initial_permeability is not None:
                            updated_porosity, updated_permeability = self.geomechanics.update_porosity_permeability(
                                initial_porosity, initial_permeability)
                            
                            # Update grid properties
                            self.simulator.grid['properties']['porosity'] = updated_porosity
                            self.simulator.grid['properties']['permeability_x'] = updated_permeability
            
            # Run thermal simulation if enabled
            if self.thermal and hasattr(self.simulator, 'simulation_results'):
                saturation = self.simulator.simulation_results.get(f'timestep_{step}', {}).get('saturation', {})
                velocity = self.simulator.simulation_results.get(f'timestep_{step}', {}).get('velocity', {})
                
                if saturation and hasattr(self.simulator, 'grid'):
                    self.thermal.solve_heat_equation(dt, saturation, velocity, self.simulator.grid)
            ...
            # Store results
            if hasattr(self.simulator, 'simulation_results') and f'timestep_{step}' in self.simulator.simulation_results:
                step_results = self.simulator.simulation_results[f'timestep_{step}']
                
                if 'pressure' in step_results:
                    results['pressure'].append(step_results['pressure'].copy())
                
                if 'saturation' in step_results:
                    results['saturation'].append({k: v.copy() for k, v in step_results['saturation'].items()})
            
            # Store specialized results
            if self.geomechanics:
                results['geomechanics'].append({
                    'stress': {k: v.copy() for k, v in self.geomechanics.stress.items()},
                    'strain': {k: v.copy() for k, v in self.geomechanics.strain.items()},
                    'displacement': {k: v.copy() for k, v in self.geomechanics.displacement.items()},
    ...
    def export_results(self, results: Dict, output_dir: str = 'results'):
        ...
        summary = {
            ...
            'timesteps': len(results['time']),
            'total_time': results['time'][-1] if results['time'] else 0.0,
            'features': {
                'geomechanics': self.geomechanics is not None,
                'thermal': self.thermal is not None,
                'compositional': self.compositional is not None,
                'dual_continuum': hasattr(self.simulator, 'dual_porosity') and self.simulator.dual_porosity
            }
        }
        ...
        # Export grid data if available
        if hasattr(self.simulator, 'grid'):
            grid_data = {
                'dimensions': {
                ...
                'cell_sizes': {
                    'dx': self.simulator.grid.get('dx', 0.0).tolist() if isinstance(self.simulator.grid.get('dx', 0.0), np.ndarray) else self.simulator.grid.get('dx', 0.0),
                    'dy': self.simulator.grid.get('dy', 0.0).tolist() if isinstance(self.simulator.grid.get('dy', 0.0), np.ndarray) else self.simulator.grid.get('dy', 0.0),
                    'dz': self.simulator.grid.get('dz', 0.0).tolist() if isinstance(self.simulator.grid.get('dz', 0.0), np.ndarray) else self.simulator.grid.get('dz', 0.0)
                }
            }
        ...
        if hasattr(self.simulator, 'wells') and self.simulator.wells:
            ...
            for well_name, well in self.simulator.wells.items():
                well_data[well_name] = {
                    ...
                    'completion': [list(comp) for comp in well.get('completion', [])]
                }
...
def create_integrated_simulator(simulator_type: str = 'tNavigator', **kwargs):
    ...
    """
    # Import commercial simulator
    from .commercial_simulator import CommercialSimulator, SimulationType
    
    # Create commercial simulator
