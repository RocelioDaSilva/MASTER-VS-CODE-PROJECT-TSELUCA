import numpy as np
import pandas as pd
from scipy.optimize import differential_evolution  # Removed minimize

# from sklearn.metrics import mean_squared_error # Unused, r2_score was not imported
import matplotlib.pyplot as plt

# import seaborn as sns # Unused
from typing import Dict, List, Optional  # Removed Tuple, Callable was not imported
...
class ProductionOptimization:
    ...
    def forecast_production(
        self, forecast_period: int, dt: float, decline_curves: Optional[Dict] = None
    ) -> Dict:
        """
        ...
        if decline_curves is not None:
            ...
            forecast = self._forecast_with_decline_curves(n_steps, dt, decline_curves)
        else:
    ...
    def _forecast_with_decline_curves(
        self, n_steps: int, dt: float, decline_curves: Dict
    ) -> Dict:
        """Previsão usando curvas de declínio."""
    ...
    def run_scenario_analysis(self, scenarios: List[Dict]) -> Dict:
        ...
        for i, scenario in enumerate(scenarios):
            ...
            forecast = self.forecast_production(
                scenario["forecast_period"], scenario["dt"]
            )

            # Calcular métricas
            metrics = self._calculate_scenario_metrics(forecast)

            results[f"scenario_{i +1}"] = {"forecast": forecast, "metrics": metrics}
    ...
    def _calculate_scenario_metrics(self, forecast: Dict) -> Dict:
        ...
        metrics = {
            ...
            "water_cut": np.mean(
                forecast["water"] / (forecast["oil"] + forecast["water"])
            ),
    ...
    def optimize_well_pattern(self, constraints: Dict) -> Dict:
        ...
        def objective(x):
            ...
            # Executar simulação
            forecast = self.forecast_production(
                constraints["forecast_period"], constraints["dt"]
            )
            ...
            wells.append(
                {
                    ...
                    "type": (
                        "producer" if result.x[2 * n_wells + i] < 0.5 else "injector"
                    ),
                }
            )
    ...
    def plot_scenario_comparison(self):
        ...
        for name, results in self.scenario_results.items():
            axes[0, 0].plot(
                results["forecast"]["time"], results["forecast"]["oil"], label=name
            )
        axes[0, 0].set_title("Produção de Óleo")
        axes[0, 0].set_xlabel("Tempo (dias)")
        axes[0, 0].set_ylabel("Taxa (bbl/d)")
        ...
        for name, results in self.scenario_results.items():
            axes[0, 1].plot(
                results["forecast"]["time"], results["forecast"]["water"], label=name
            )
        axes[0, 1].set_title("Produção de Água")
        axes[0, 1].set_xlabel("Tempo (dias)")
        axes[0, 1].set_ylabel("Taxa (bbl/d)")
        ...
        for name, results in self.scenario_results.items():
            axes[1, 0].plot(
                results["forecast"]["time"], results["forecast"]["gas"], label=name
            )
        axes[1, 0].set_title("Produção de Gás")
        axes[1, 0].set_xlabel("Tempo (dias)")
        axes[1, 0].set_ylabel("Taxa (Mscf/d)")
        ...
        axes[1, 1].bar(recovery_factors.keys(), recovery_factors.values())
