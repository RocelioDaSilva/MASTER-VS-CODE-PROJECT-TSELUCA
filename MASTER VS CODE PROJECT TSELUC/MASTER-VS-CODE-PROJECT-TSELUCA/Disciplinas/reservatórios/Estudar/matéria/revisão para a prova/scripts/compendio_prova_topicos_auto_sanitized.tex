...
\item \textbf{Razão de solubilidade ($R_s$):} quantidade de gás dissolvido por unidade de óleo (scf/STB ou $\mathrm{m}^3/\mathrm{m}^3$). No ponto de bolha, $R_s$ corresponde ao máximo gás dissolvido antes do aparecimento de gás livre.
...
\item $\rho$: densidade (kg/$\mathrm{m}^3$).
...
- Método alternativo (método de Arquímedes) e outros exemplos estão disponíveis em $exercicios_{resolvidos}$.md e no material do Cap.3.
\section{Capítulo 1 — Afirmações V/F (prontas)}
...
Saída esperada aproximada (exemplo que usamos manualmente): Z ≈ 0.948, $c_{g}$ ≈ 2.19e-7 Pa$^{-1}$.
