...
export const FinancialAnalysis: React.FC<FinancialAnalysisProps> = ({ results, communities }) => {
  ...
  const calculateAllMetrics = async () => {
    ...
    for (const community of communities) {
      ...
      // Client-side financial calculation (no backend dependency)
      const annualEnergyKwh = community.ghi * 365 * (solarParams.wattage / 1000) * solarParams.efficiency;
      const totalLifetimeCost = solarParams.capitalCost + solarParams.omCost * solarParams.lifetime;
      const lcoe = annualEnergyKwh > 0 ? totalLifetimeCost / (annualEnergyKwh * solarParams.lifetime) : 0;
      const electricityPrice = 0.12; // $/kWh assumed tariff
      const totalLifetimeRevenue = annualEnergyKwh * solarParams.lifetime * electricityPrice;
      const roi = solarParams.capitalCost > 0 ? ((totalLifetimeRevenue - totalLifetimeCost) / solarParams.capitalCost) * 100 : 0;
      const annualSavings = annualEnergyKwh * electricityPrice - solarParams.omCost;
      const paybackYears = annualSavings > 0 ? solarParams.capitalCost / annualSavings : solarParams.lifetime;

      metrics.set(community.id, {
        lcoe,
        roi,
