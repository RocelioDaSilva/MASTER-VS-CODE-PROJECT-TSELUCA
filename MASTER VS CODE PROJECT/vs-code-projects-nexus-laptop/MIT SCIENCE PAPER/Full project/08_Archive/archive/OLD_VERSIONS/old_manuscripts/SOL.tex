...
\[
\text{LCOE} = \frac{18.700 + \sum_{t=1}^{20} \frac{400}{(1,08)^t}}{0,70 \times 10 \times \sum_{t=1}^{20} \frac{8.760}{(1,08)^t}}  
\approx \frac{18.700 + 3.690}{54.600} \approx \$0.206\text{ USD/kWh}
\]

Este valor de \textbf{USD 0,20--0,21/kWh é competitivo com}:
\begin{itemize}
...
\begin{enumerate}
\item \textbf{Erro na Estimativa de Irradiação}: Se NASA POWER subestima GHI (especialmente em áreas com nuvem orográfica variável), sistema dimensionado pode ficar sub-potência. Risco: Deceção comunitária, desconexão, retorno a diesel.
...
Desde que Fase 3 (Midterm) detectar desvios >60\% entre projeção e observado em indicadores sociais importamtes (ex., cobertura vacinal, horas de estudo), protocolo requere:
...
\begin{itemize}
    \item \textbf{Dados desatualizados:} O censo mais recente é de 2014, não refletindo completamente a dinâmica populacional atual; Censo 2024 em processamento, esperado Q2 2026
    \item \textbf{Fatores não quantificados:} Aceitação comunitária, capacidade de pagamento individual, dinâmicas de governança local foram incluídos principalmente através de consulta qualitativa, não redes bayesianas preditivas
    \item \textbf{Resolução espacial:} Alguns dados (como VIIRS) têm resolução moderada (375m), limitando análise em comunidades muito pequenas (<300 hab); mitigado por GPS comunitário em baseline
    \item \textbf{Validação de campo limitada:} A análise baseou-se principalmente em dados secundários; protocolo de validação estruturado (Fase 3) será executado em Q3--Q4 2026
\end{itemize}
