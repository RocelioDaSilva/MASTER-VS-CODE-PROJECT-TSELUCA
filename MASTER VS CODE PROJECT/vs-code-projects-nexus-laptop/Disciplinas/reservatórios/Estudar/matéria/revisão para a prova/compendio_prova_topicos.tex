...
% Monospaced font with wide glyph coverage for code/Verbatim
...
\item **Razão de solubilidade ($R_s$):** quantidade de gás dissolvido por unidade de óleo (scf/STB ou $\mathrm{m}^3/\mathrm{m}^3$). No ponto de bolha, $R_s$ corresponde ao máximo gás dissolvido antes do aparecimento de gás livre.
...
$$
R_s\ (\mathrm{m}^3/\mathrm{m}^3) = \frac{400\times0.0283168}{0.1589873} \approx 71.3\ \mathrm{m}^3/\mathrm{m}^3
$$
...
\begin{itemize}
\item $B_o$: fator de volume de formação do óleo (bbl/STB ou $\mathrm{m}^3/\mathrm{m}^3$).
\item $R_s$: razão gás/óleo dissolvido (scf/STB ou $\mathrm{m}^3/\mathrm{m}^3$).
...
\begin{itemize}
\item Embebição: processo onde a fase molhante invade a rocha, reduzindo saturação da fase não‑molhante.
...
\begin{enumerate}
\item Dado: Área = 200 acres; $h_{net} = 30$ ft; $\phi = 0.18$; $S_{wi} = 0.25$; $B_o = 1.2$ bbl/STB. Calcule OOIP (use fórmula prática).
...
\item **Área efetiva ($A_{eff}$):** área do reservatório considerada produtiva após aplicar cut‑offs e mapas de net.
\end{itemize}
...
\begin{enumerate}
\item Redija uma dissertação técnica sobre ``Incertezas no método volumétrico'' (máx. 1 página).
...
\begin{comment}
\subsection{prova_banco_questoes_toda_matriz.md}
\begin{verbatim}
...
1. Calcule $B_o$ e $R_s$ para ensaio PVT simplificado: $V_{res}=1.20$ bbl; $V_{surf}=1.00$ STB; $R_s=400$ scf/STB. (a) $B_o$ em bbl/STB; (b) converta $R_s$ para $\mathrm{m}^3/\mathrm{m}^3$).
...
\subsection{prova_banco_questoes_cap1-4.md}
\begin{verbatim}
...
1. Calcule \(B_o\) e \(R_s\) para um ensaio PVT simplificado (caso modelo):  
	...
	- (a) Calcule \(B_o\) em bbl/STB; (b) converta \(R_s\) para $\mathrm{m}^3/\mathrm{m}^3$ (use 1 scf = 0.0283168 $\mathrm{m}^3$; 1 STB = 0.1589873 $\mathrm{m}^3$).
...
\subsection{provas_calc_cap2.md}
\begin{verbatim}
...
1) Ensaio PVT: $V_{res}=1.20$ bbl; $V_{surf}=1.00$ STB; $R_s=400$ scf/STB.
	...
	b) Converta $R_s$ para $\mathrm{m}^3/\mathrm{m}^3$ (use 1 scf = 0.0283168 $\mathrm{m}^3$; 1 STB = 0.1589873 $\mathrm{m}^3$).

2) Use a fórmula prática para OOIP:
	$$OOIP = \frac{7758\,A\,h\,\phi\,(1-S_{wi})}{B_o}$$
	Calcule OOIP para: A = 150 acres; h = 25 ft; $\phi = 0.16$; $S_{wi} = 0.20$; $B_o = 1.10$.
...
8) Conversão e fator de formação: calcule OGIP (scf) para A = 100 acres, h = 20 ft, $\phi = 0.12$, $S_{wi} = 0.25$ e $B_g = 0.005\ \mathrm{m}^3/\mathrm{Sm}^3$ (adapte unidades conforme necessário).
...
\subsection{provas_calc_cap3.md}
\begin{verbatim}
...
\subsection{provas_dissertacao_cap4.md}
\begin{verbatim}
...
1) Discuta as principais fontes de incerteza em uma estimativa volumétrica de OOIP (A, $h_{net}$, $\phi$, $S_{wi}$, $B_o$). Proponha estratégias práticas para reduzir cada fonte de incerteza.
...
\subsection{exercicios_resolvidos.md}
\begin{verbatim}
...
\clearpage
\section*{Notas}
Os blocos acima reúnem os exercícios e tarefas extraídos dos ficheiros Markdown presentes na pasta `Estudar/matéria/revisão para a prova`. Alguns itens podem duplicar conteúdos já incluídos nas secções de consolidação dos capítulos; mantive cada fonte como subseção identificada para controlo e revisão.
