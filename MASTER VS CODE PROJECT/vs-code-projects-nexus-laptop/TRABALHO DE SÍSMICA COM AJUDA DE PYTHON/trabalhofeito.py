...
import pandas as pd
import matplotlib
# Usar backend 'Agg' para evitar dependência de interface gráfica (Tk) ao gerar figuras
matplotlib.use('Agg')
...
with pd.ExcelWriter(output_excel, engine="xlsxwriter") as writer:
    ...
    # geometria NMO: t^2 = t0^2 + x^2 / V^2
    # aqui escrevemos as colunas auxiliares no Excel:
    # - x^2 (m^2) = offset^2
    # - t^2 (s^2) = (t_ms/1000)^2
    # Estes são utilizados na regressão linear t^2 vs x^2 para obter
    # intercepto = t0^2 e slope = 1/V^2 (ver seções teóricas no relatório).
    ...
    for idx, t_range in enumerate(rng_tsq_list, start=1):
        ...
        # No Excel:
        # INTERCEPT(t^2_range, x^2_range) -> estimativa de t0^2 (intercepto)
        # SLOPE(t^2_range, x^2_range) -> estimativa de m = 1/V^2
        # então t0 = sqrt(intercept) e V = 1/sqrt(slope)
        ...
        # Vnmo obtido a partir da inclinação m: V = 1/sqrt(m)
    ...
    for i in range(2, 6):
        ...
        # Fórmula de Dix (usando tempos t0 em segundos):
        # Vint_i = sqrt( (V_rms_i^2 * t_i - V_rms_{i-1}^2 * t_{i-1}) / (t_i - t_{i-1}) )
        # No Excel usamos as células de Vnmo (tratadas como Vrms aqui) e t0_s
    ...
    # thickness (camada 1) = V_int1 * (t0_1 / 2)  (t0 em s -> /2 para tempo 1-way)
    ...
    for i in range(nrows):
        ...
        # Fórmula de correção NMO aplicada por célula no Excel:
        # t0 = sqrt( t^2 - x^2 / V^2 )  (t em s, x em m, V em m/s)
        # Aqui usamos tempos originais em ms, por isso convertendo (t_ms/1000)^2
        # Se a expressão dentro da raiz for <=0, escrevemos 0 (mute)
...
# Salva tabela numérica de resultados como CSV para inspeção rápida (valores calculados
# numericamente no Python: t0s_ms, vnmos, v_int_nums, thicknesses, depths)
csv_out = Path(OUTPUT_DIR) / f"{input_file.stem}_results.csv"
res_df.to_csv(csv_out, index=False)
print("Resultados numéricos salvos em:", csv_out.resolve())
...
with PdfPages(output_pdf) as pdf:
    ...
    # Página com resumo mínimo (fornecido pelo utilizador)
    fig = plt.figure(figsize=(8.27, 11.69))
    fig.suptitle("Resumo mínimo - Procedimento NMO/Dix e entregáveis", fontsize=14)
    txt = (
        "Dados necessários:\n"
        "- Offset (x) em metros\n"
        "- Tempos de reflexão t(x) em ms\n\n"
        "Relação NMO: t(x)^2 = t0^2 + x^2 / V^2\n\n"
        "Procedimento essencial:\n"
        "1) Converter t[ms]->t[s] (t/1000); calcular x^2 e t^2\n"
        "2) Ajuste linear t^2 = intercept + slope*x^2 -> t0 = sqrt(intercept); Vnmo=1/sqrt(slope)\n"
        "3) Dix: Vint = sqrt( (Vrm_i^2 * t_i - Vrms_{i-1}^2 * t_{i-1})/(t_i - t_{i-1}) )\n"
        "4) Espessura: dz = Vint * (t_i - t_{i-1})/2; profundidade = soma(dz)\n\n"
        "Gráficos obrigatórios por reflector:\n"
        "- t^2 vs x^2 (scatter + regressão)\n"
        "- t vs x (pontos + curva hiperbólica NMO)\n\n"
        "Cuidados mínimos:\n"
        "- intercepto<0 -> rever offsets ou t0~0; Dix inválido se t_i<=t_{i-1}; assume reflectores horizontais\n\n"
        "Entrega final: tabela (t0, Vnmo, Vint, espessura, profundidade) + 10 gráficos (5x2)."
    )
    fig.text(0.05, 0.95, txt, va='top', wrap=True, fontsize=10)
