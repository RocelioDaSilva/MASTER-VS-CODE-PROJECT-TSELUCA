...
# from scipy.stats import norm # Unused, t is not imported here either
...
class ReservesEvaluation:
    ...
    def calculate_reserves(
        ...
        confidence_levels: Dict[ReserveCategory, float] = None,
    ) -> Dict[ReserveCategory, ReservesEstimate]:
        """
        ...
        oil_rf = self._calculate_recovery_factor(
            production_data, petrophysical_data, "oil"
        )
        gas_rf = self._calculate_recovery_factor(
            production_data, petrophysical_data, "gas"
        )
        ...
        for category in ReserveCategory:
            ...
            npv = self._calculate_npv(oil_volume, gas_volume, economic_params)
            ...
            reserves[category] = ReservesEstimate(
                ...
                recovery_factor=(
                    oil_rf
                    if category == ReserveCategory.PROVED
                    else oil_rf * confidence
                ),
    ...
    def _calculate_decline_rate(
        self, production_data: pd.DataFrame, fluid_type: str
    ) -> float:
        """
    ...
    def _calculate_npv(
        self, oil_volume: float, gas_volume: float, economic_params: Dict
    ) -> float:
        """
    ...
    def run_scenario_analysis(self, base_case: Dict, scenarios: List[Dict]) -> Dict:
        """
    ...
    def generate_technical_report(self) -> str:
        ...
        report.append(
            f"- Receita Total: ${self.economic_results['total_revenue']:,.2f}\n"
        )
    ...
    def generate_reserves_report(
        self, reserves: Dict[ReserveCategory, ReservesEstimate], output_path: str
    ):
        """
        ...
        report.append("-" * 50)
        report.append(
            f"{'Categoria':<10} {'Óleo (MMbbl)':<15} {'Gás (Bcf)':<15} {'NPV (MM$)':<15}"
        )
        report.append("-" * 50)
