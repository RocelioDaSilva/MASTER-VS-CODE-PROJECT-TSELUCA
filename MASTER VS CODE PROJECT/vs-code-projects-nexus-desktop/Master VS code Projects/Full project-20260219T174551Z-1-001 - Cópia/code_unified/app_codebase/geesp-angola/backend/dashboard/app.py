...
# ============================================================================
# FIX SYS.PATH - MUST HAPPEN BEFORE OTHER IMPORTS
# ============================================================================
# When running from dashboard subdirectory, add parent to path
...
# Initialize project paths and logging (centralized)
...
@st.cache_data
def _load_communities(path: str):
    """Cache communities CSV to avoid repeated disk I/O on each page visit."""
    p = Path(path)
    return pd.read_csv(p) if p.exists() else pd.DataFrame()


communities_df = _load_communities(str(communities_path))
...
if page == "🏠 Início":
    ...
    if not communities_df.empty:
        ...
        if selected_comm != "Visão Geral":
            ...
            map_lat, map_lon = UIConstants.MAP_CENTER_LAT, UIConstants.MAP_CENTER_LON
    else:
        selected_comm = "Visão Geral"
        map_lat, map_lon = UIConstants.MAP_CENTER_LAT, UIConstants.MAP_CENTER_LON

    # Cria mapa base
    if not HAS_FOLIUM:
        st.info("Instale 'folium' e 'streamlit-folium' para visualizar o mapa interativo.")
    else:
        zoom_level = UIConstants.MAP_ZOOM_OVERVIEW if selected_comm == "Visão Geral" else UIConstants.MAP_ZOOM_DETAIL
        m = folium.Map(
    ...
    if uploaded_files:
        ...
        for uf in uploaded_files:
            ...
            if size_mb > MAX_FILE_SIZE_MB:
                st.error(f"❌ {uf.name}: {size_mb:.1f} MB excede o limite de {MAX_FILE_SIZE_MB} MB")
                continue
            # Check TIFF magic bytes
            header = uf.read(4)
            uf.seek(0)
            if header[:2] not in (b"II", b"MM"):
                st.error(f"❌ {uf.name}: não é um arquivo TIFF válido")
                continue
            valid_files.append(uf)

        if not valid_files:
            st.warning("Nenhum arquivo válido para processar.")
        else:
            st.success(f"✓ {len(valid_files)} arquivo(s) válido(s) carregado(s)")
        ...
        with col2:
            ...
            if HAS_PLOTLY:
                ...
                st.info("Instale 'plotly' para ver gráficos interativos.")
...
# ============================================================================
    ...
    for criterion, default in default_weights.items():
        weights[criterion] = st.sidebar.slider(
            f"{criterion}",
            MCDAConstants.WEIGHT_MIN,
            MCDAConstants.WEIGHT_MAX,
            default,
    ...
    if st.button("▶️ Executar Análise MCDA", use_container_width=True):
        ...
        with st.spinner("Processando dados..."):
            ...
            map_keys = {
                ...
                "Luminosidade Noturna": "mapa_luminosidade_noturna",
            }
            ...
            if skipped:
                logger.warning(f"Skipped {len(skipped)} missing maps: {skipped}")
                st.warning(f"Mapas não encontrados: {', '.join(skipped)}")
                ...
                with col2:
                    ...
                    try:
                        ...
                        if irr_map is not None:
                            ...
                            annual_irr = LCOEConstants.DEFAULT_ANNUAL_IRRADIANCE
        ...
        # Store normed maps in session state so sensitivity can access them
        st.session_state["mcda_normed"] = normed
        st.session_state["mcda_weights"] = weights
        st.session_state["mcda_total"] = total

    # Sensitivity analysis (outside main button so it's reachable)
    if "mcda_normed" in st.session_state and st.button("▶️ Executar Sensibilidade para critério selecionado"):
        normed = st.session_state["mcda_normed"]
    ...
    with col2:
        ...
        if st.button("Calcular LCOE", use_container_width=True, type="primary"):
            ...
            if HAS_PLOTLY:
                ...
                st.dataframe(comparison[["technology_name", "lcoe_usd_per_kwh"]], use_container_width=True)
