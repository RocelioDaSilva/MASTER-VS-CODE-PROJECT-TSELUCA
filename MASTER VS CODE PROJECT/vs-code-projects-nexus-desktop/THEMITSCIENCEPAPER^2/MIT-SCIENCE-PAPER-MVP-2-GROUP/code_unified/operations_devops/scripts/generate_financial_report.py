...
if str(proj_root) not in path:
    path.insert(0, str(proj_root))
from utils.constants import FinancialConstants
...
# Color scheme for charts (from FinancialConstants)
COLOR_PALETTE = FinancialConstants.COLOR_PALETTE
...
class FinancialAnalyzer:
    ...
    def calculate_lcoe(self) -> float:
        ...
        project_lifetime = self.config.get('project_lifetime', FinancialConstants.PROJECT_LIFETIME_DEFAULT)
    ...
    def calculate_irr(self) -> float:
        ...
        # Newton-Raphson
        rate = FinancialConstants.DISCOUNT_RATE_INIT
        for _ in range(FinancialConstants.IRR_MAX_ITERATIONS):
            f = npv(rate)
            df_rate = rate * FinancialConstants.IRR_TOLERANCE
