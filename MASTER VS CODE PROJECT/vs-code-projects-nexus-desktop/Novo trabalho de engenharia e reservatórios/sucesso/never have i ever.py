...
class EORSelectionApp:
    ...
    def setup_ui(self):
        ...
        for label_text, key, min_val, max_val in parameters:
            ...
            slider = ctk.CTkSlider(
                ...
                from_=min_val, 
                to=max_val,
                command=lambda val, e=entry, k=key: self.on_slider_change(val, e, k)
            )
    ...
    def setup_results_area(self):
        ...
        self.results_text = ctk.CTkTextbox(self.recommendations_tab, height=200)
        ...
        self.economic_text = ctk.CTkTextbox(self.economic_tab, height=200)
    ...
    def on_slider_change(self, value, entry, key):
        ...
        try:
            ...
            for key in self.default_params.keys():
                try:
                    ...
                    messagebox.showerror("Erro", f"Valor inválido para {key}")
                    return
    ...
    def calculate_npv(self, params, years=10):
        ...
        for year in range(1, years + 1):
            discounted_cf = annual_cash_flow / ((1 + params["discount_rate"]/100) ** year)
    ...
    def display_results(self, params, recommendations, npv, cash_flows):
        ...
        for key, value in params.items():
            result_text += f"{key.replace('_', ' ').title()}: {value:.2f}\n"
        ...
        economic_text += f"\nRetorno sobre Investimento (ROI): {(npv/params['capital_cost']*100):.1f}%\n"
    ...
    def update_graphs(self, recommendations, npv, cash_flows):
        ...
        colors = []
        for method, score in recommendations["scores"].items():
                ...
                if not df.empty:
                    ...
                    for col in df.columns:
                        ...
                        if col_lower in self.entries:
                            ...
                            self.entries[col_lower].insert(0, str(df[col].iloc[0]))
    ...
    def export_results(self):
        ...
        if file_path:
            try:
                ...
                content += "\nRecomendações EOR:\n"
                for method, score in recommendations["scores"].items():
                    status = recommendations["recommendations"][method]
                    content += f"  {method}: {score:.1f}% - {status}\n"
                
                content += f"\nAnálise Econômica:\n"
                content += f"  NPV: ${npv:,.2f}\n"
                content += f"  ROI: {(npv/params['capital_cost']*100):.1f}%\n"
